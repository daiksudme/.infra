...
export async function manageFoundation(operation, ops) {
  ...
  for (const { address, change } of plan.resource_changes) {
    const target = targets.find((item) => [item.address, item.privateAddress].includes(address));
    if (!target || (address === target.address && change.actions.includes('create'))) throw new Error('Unexpected resource or bucket recreation plan');
  }
